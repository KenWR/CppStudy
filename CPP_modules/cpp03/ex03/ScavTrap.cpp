...
ScavTrap::ScavTrap(const std::string &name)
    : ClapTrap(name, this->default_hit_points_, this->default_energy_points_,
...
ScavTrap::ScavTrap(const ScavTrap &copy)
    : ClapTrap(copy.name_, copy.energy_points_, copy.hit_points_,
               copy.attack_damage_) {
  std::cout << "\033[33mScavTrap " << this->name_
...
void ScavTrap::printAttack(const std::string &name, const std::string &target,
                           unsigned int damage) {
...
void ScavTrap::printNotEnoughEnergy(const std::string &name) {
  std::cout << "ScavTrap " << (name) << " do not have enough energy"
            << std::endl;
}

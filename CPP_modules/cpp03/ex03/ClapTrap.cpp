...
ClapTrap::ClapTrap()
    : name_("Strange"), hit_points_(this->default_hit_points_),
...
ClapTrap::ClapTrap(const std::string &name)
    : name_(name), hit_points_(this->default_hit_points_),
      energy_points_(this->default_energy_points_),
      attack_damage_(this->default_attack_damage_) {
  std::cout << "\033[33mClapTrap " << this->name_
            << " Constructor works done\033[0m" << std::endl;
...
ClapTrap::ClapTrap(const std::string &name, unsigned int hit_points,
    ...
    : name_(name), hit_points_(hit_points), energy_points_(energy_points),
...
ClapTrap::ClapTrap(const ClapTrap &copy)
    : name_(copy.name_), hit_points_(copy.hit_points_),
      energy_points_(copy.energy_points_), attack_damage_(copy.attack_damage_) {
  std::cout << "\033[33mClapTrap " << this->name_
...
void ClapTrap::printAttack(const std::string &name, const std::string &target,
                           unsigned int damage) {
...
void ClapTrap::printNotEnoughEnergy(const std::string &name) {
  std::cout << "ClapTrap " << (name) << " do not have enough energy"
            << std::endl;
}

  ...
  for (int i = 0; i < PHONE_BOOK_SIZE; i++) {
    ...
    if (this->phone_book_[i].GetFirstName().length() > 10) {
      ...
      std::cout << std::setw(10) << std::setfill(' ') << std::right
                << this->phone_book_[i].GetFirstName();
      std::cout << "|";
    }
    if (this->phone_book_[i].GetLastName().length() > 10) {
      ...
      std::cout << std::setw(10) << std::setfill(' ') << std::right
                << this->phone_book_[i].GetLastName();
      std::cout << "|";
    }
    if (this->phone_book_[i].GetNickName().length() > 10) {
      ...
      std::cout << std::setw(10) << std::setfill(' ') << std::right
                << this->phone_book_[i].GetNickName();
      std::cout << "|";
    }
    std::cout << std::endl;
    PrintLine();
...
void PhoneBook::PrintContact() {
  ...
  if (0 <= index && index <= PHONE_BOOK_SIZE - 1) {
    std::cout << "First name: " << this->phone_book_[index].GetFirstName()
              << std::endl;
    std::cout << "Last name: " << this->phone_book_[index].GetLastName()
              << std::endl;
    std::cout << "Nickname: " << this->phone_book_[index].GetNickName()
              << std::endl;
    std::cout << "Phone number: " << this->phone_book_[index].GetPhoneNumber()
              << std::endl;
    std::cout << "Darkest secret: "
              << this->phone_book_[index].GetDarkestSecret() << std::endl;
  } else {
